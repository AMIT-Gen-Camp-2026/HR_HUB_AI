...
_VIDEO_MIME_TYPES = {
    ...
    # Some browsers omit precise video MIME types for local files.
...
def validate_video_mime_type(mimetype: str | None) -> None:
    """Reject an explicitly non-video MIME type while tolerating generic browser uploads."""
    if mimetype and mimetype.lower() not in _VIDEO_MIME_TYPES:
        raise FileValidationError("Uploaded content is not a supported video MIME type.")


def validate_video_content(filepath: str, ext: str) -> None:
    """Check the expected container signature before untrusted media decoding."""
    try:
        with open(filepath, "rb") as file:
            header = file.read(16)
    except OSError as error:
        raise FileValidationError("Could not read the uploaded video.") from error

    is_iso_container = len(header) >= 8 and header[4:8] == b"ftyp"
    is_ebml_container = header.startswith(b"\x1aE\xdf\xa3")
    valid = is_iso_container if ext in {".mp4", ".mov"} else is_ebml_container
    if not valid:
        raise FileValidationError("Video content does not match its declared format.")
