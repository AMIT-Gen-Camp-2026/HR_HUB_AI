...
The Streamlit **Video Demo** tab accepts uploaded `.mp4`, `.mov`, `.mkv`, and `.webm`
files and returns a transcript for Arabic, English, or mixed Arabic/English speech. It
uses the multilingual `faster-whisper` runtime; the default `small` model is practical
for a local demo and loads only on the first transcription request.

```bash
VIDEO_ASR_MODEL=small
VIDEO_MAX_SIZE_MB=200
VIDEO_ASR_DEVICE=auto
VIDEO_ASR_COMPUTE_TYPE=auto
```

`auto` uses CUDA with FP16 when CTranslate2 can access a compatible GPU and otherwise
uses quantized CPU inference. The model's PyAV dependency bundles FFmpeg libraries, so
an FFmpeg command-line installation is not required. The first real request downloads
the configured model weights; tests mock the ASR service and do not download a model.

Example request:

```bash
curl -X POST http://127.0.0.1:5000/api/v1/video/transcribe -F "file=@demo.mp4"
```

Example response shape:

```json
{"success": true, "language": "ar", "text": "...", "segments": [{"start": 0.0, "end": 4.2, "text": "..."}]}
```
