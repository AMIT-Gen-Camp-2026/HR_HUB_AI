...
# --- إضافة Sprint 2 ---
#
# tests/conftest.py و app/api/routes_cv.py الجديدين بيتوقعوا كائن
# اسمه PromptRegistry يتبني بـ prompts_dir واحد (get_settings().prompts_dir)
# ويتحط في request.app.state.prompts. الكلاس ده دلوقتي "غلاف" بس حوالين
# build_prompt() و SYSTEM_PROMPT اللي فوق - مفيش أي تعديل في نص الـ
# prompt نفسه ولا في آلية الـ randomized delimiters اللي بتحمي من
# الـ prompt injection. لو لاحقًا حبينا نحمّل نسخة versioned من
# app/prompts/templates/cv_extract.v1.jinja بدل النص الثابت، ده هيبقى
# المكان الوحيد اللي هيتغيّر - من غير ما نغيّر توقيع الكلاس من برّه.
# ============================================================


class PromptRegistry:
    """غلاف حول build_prompt()/SYSTEM_PROMPT - نفس المنطق الأمني، بدون تعديل."""

    def __init__(self, prompts_dir: str | Path = "app/prompts/templates") -> None:
        self.prompts_dir = Path(prompts_dir)

    def build_cv_extraction_prompt(self, cv_text: str) -> tuple[str, str]:
        """بترجع (system_prompt, user_prompt) لاستخراج بيانات الـ CV. راجع build_prompt()."""
        return build_prompt(cv_text)

    def __len__(self) -> int:
        """
        عدد الـ prompts المحمّلة - app/api/routes_health.py بيستخدمها في /health
        (len(request.app.state.prompts)) كمؤشر سريع إن الـ prompts اتحمّلت صح.

        بتعدّ ملفات .jinja الموجودة فعليًا في prompts_dir (زي cv_extract.v1.jinja،
        skill_infer.v1.jinja). لو المجلد مش موجود أو فاضي، بترجع 1 - عشان
        SYSTEM_PROMPT الثابت اللي فوق ده أصلاً "prompt واحد" شغال حتى من
        غير أي ملف .jinja.
        """
        if self.prompts_dir.is_dir():
            jinja_count = len(list(self.prompts_dir.glob("*.jinja")))
            if jinja_count > 0:
                return jinja_count
        return 1